            ...
            data.map((odont)=>{
                odontologoRow.innerHTML += `
                    <tr>
                        ...
                        <td>
                            <button id="edit" onclick="handleEdit(${odont.id})" class="btn btn-secondary"><i class="fa-solid fa-file-pen"></i></button>
                            <button id="btnDelete" onclick="deleteOdontologo(${odont.id})" class="btn btn-danger"><i class="fa-solid fa-trash-can"></i></button>
                        </td>
                    </tr>`
            })
        })
...
function addOdontologo(dataOdontologo){
    ...
    fetch(`${API_BASE}/odontologos/registrar`, settings)
        .then(res => {
            if(res.status != 201) throw new Error('No se pudo agregar el odontólogo.') 
            return res.json();
        })
        .then(data => {
            listOdontologos();
        ...
        odontologoRow.innerHTML += `
            <tr>
                ...
                <td>
                    ...
                    <button id="btnDelete" onClick="deleteOdontologo(${data.id})"></button>
                </td>
            </tr>`
        resetUploadForm();
    }).catch((error) => {
...
const deleteOdontologo = (id) => {
    ...
    .then((res) => {
       console.log(res.status)
        ...
        if(res.status != 204) throw new Error('No se pudo eliminar el Odontólogo')
    })
    .then((data) => {
        listOdontologos();
    })
...
if(btnAdd){
    try {
        btnAdd.addEventListener('click', (e) => {
            e.preventDefault();
            let nombreOdontologo = document.querySelector("#nombre"),
            apellidoOdontologo = document.querySelector("#apellido"),
